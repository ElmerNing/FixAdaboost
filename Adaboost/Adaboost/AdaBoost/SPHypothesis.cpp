...
{  
  for (int i = 0; i < (int)m_vDims.size(); i++)
  {
...
bool CSPHypothesis::LoadFromFile(FILE* in_File)
{
  ...
  for (int i = 0; i < N; i++)
  {
    float DimBuffer,
          ThreshBuff,
          SignumBuff;
    if(!fscanf_s(in_File, "%f %f %f", &DimBuffer, &ThreshBuff, &SignumBuff))
      return false;
...
bool CSPHypothesis::LoadFromString(const char* Data)
{
  ...
  for (int i = 0; i < N; i++)
  {
    float DimBuffer,
          ThreshBuff,
          SignumBuff;
    if(!sscanf_s(Data, "%f %f %f", &DimBuffer, &ThreshBuff, &SignumBuff))
      return false;
    m_vDims[i] = (int) DimBuffer - 1;
    m_vThresholds[i] = ThreshBuff;
    m_vSignums[i] = SignumBuff;
